...
QString Settings::profileDir(QString profile, bool useFallback) {
    ...
    QString dirName = QStandardPaths::writableLocation(QStandardPaths::ConfigLocation)
                      + QStringLiteral("/panda-files/") + profile;
    QDir dir(dirName);
    ...
    if (!dir.exists() && useFallback) {
        ...
        for (const auto &thisConf : confList) {
            fallbackDir = thisConf + QStringLiteral("/panda-files/") + profile;
...
bool Settings::loadFile(QString filePath)
{
    ...
    wallpaperMode_ = wallpaperModeFromString(settings.value(QStringLiteral("WallpaperMode"), "stretch").toString());
    wallpaper_ = settings.value(QStringLiteral("Wallpaper"), "/usr/share/wallpapers/panda/gradienta-LeG68PrXA6Y-unsplash.jpg").toString();
