...
#include "lib/folderitemdelegate.h"
#include "lib/cachedfoldermodel.h"
#include "lib/folderview_p.h"
...
void DesktopWindow::createTrashShortcut(int items) {
    ...
    g_key_file_set_string(kf, "Desktop Entry", "Exec", "panda-files trash:///");
    // icon
...
void DesktopWindow::createHomeShortcut() {
    GKeyFile* kf = g_key_file_new();
    g_key_file_set_string(kf, "Desktop Entry", "Type", "Application");
    g_key_file_set_string(kf, "Desktop Entry", "Exec", Fm::CStrPtr(g_strconcat("panda-files ", Fm::FilePath::homeDir().toString().get(), nullptr)).get());
...
void DesktopWindow::createComputerShortcut()
{
    GKeyFile* kf = g_key_file_new();
    g_key_file_set_string(kf, "Desktop Entry", "Type", "Application");
    g_key_file_set_string(kf, "Desktop Entry", "Exec", "panda-files computer:///");
...
void DesktopWindow::createNetworkShortcut() {
    GKeyFile* kf = g_key_file_new();
    g_key_file_set_string(kf, "Desktop Entry", "Type", "Application");
    g_key_file_set_string(kf, "Desktop Entry", "Exec", "panda-files network:///");
...
void DesktopWindow::trustOurDesktopShortcut(std::shared_ptr<const Fm::FileInfo> file) {
    ...
    const QString fileName = QString::fromStdString(file->name());
    auto homeExec = Fm::CStrPtr(g_strconcat("panda-files ", Fm::FilePath::homeDir().toString().get(), nullptr));
    const char* execStr = fileName == QLatin1String("trash-can.desktop") && ds.contains(QLatin1String("Trash")) ? "panda-files trash:///" :
                          fileName == QLatin1String("user-home.desktop") && ds.contains(QLatin1String("Home")) ? homeExec.get() :
                          fileName == QLatin1String("computer.desktop") && ds.contains(QLatin1String("Computer")) ? "panda-files computer:///" :
                          fileName == QLatin1String("network.desktop") && ds.contains(QLatin1String("Network")) ? "panda-files network:///" : nullptr;
    if(execStr) {
        GKeyFile* kf = g_key_file_new();
        if(g_key_file_load_from_file(kf, file->path().toString().get(), G_KEY_FILE_NONE, nullptr)) {
            Fm::CStrPtr str{g_key_file_get_string(kf, "Desktop Entry", "Exec", nullptr)};
